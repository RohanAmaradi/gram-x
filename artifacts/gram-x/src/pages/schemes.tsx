  ...
  return (
    <div className="p-4 space-y-6">
      <header className="mb-6">
        <h1 className="text-2xl font-bold text-primary mb-1">{t.schemes.title}</h1>
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          >
            <Card className="overflow-hidden border-border/50" data-testid={`card-scheme-${scheme.id}`}>
              ...
              <CardContent className="pt-4 space-y-3">
                ...
                <div className="bg-primary/5 rounded-lg p-3 text-sm">
                  <span className="font-semibold text-primary block mb-1">{t.schemes.benefits}:</span>
                  {scheme.benefit}
                </div>

                <div className="text-sm">
                  <span className="font-medium text-foreground block mb-1">{t.schemes.eligibility}:</span>
              ...
              <CardFooter className="bg-muted/10 border-t border-border/50 pt-4 flex flex-col sm:flex-row gap-3 justify-between items-center">
                <div className="flex items-center text-xs text-muted-foreground font-medium w-full sm:w-auto">
                  ...
                  {scheme.deadline
                    ...
                    : t.schemes.noDeadline}
                </div>
                {scheme.applicationLink && (
                  <Button className="w-full sm:w-auto bg-primary text-primary-foreground" asChild data-testid={`btn-apply-${scheme.id}`}>
                    <a href={scheme.applicationLink} target="_blank" rel="noopener noreferrer">
                      {t.schemes.applyNow} <ExternalLink className="w-4 h-4 ml-2" />
                    </a>
                  </Button>
                )}
              </CardFooter>
